...
public class KafkaFileConsumer {
    ...
    public void consumeFileParts(@Payload byte[] fileContent,
        ...
        try {
            if (fileParts.containsKey(key)) {
                ...
                final File tempFile = Files.createTempFile(tempDirectory, "split_" + currentFilePart + "_", ".json").toFile();
                FileOutputStream fos = new FileOutputStream(tempFile);
                try (fos) {
                    fos.write(fileContent);
                }

                fileParts.get(key).add(new FilePart(tempFile.getAbsolutePath(), Integer.parseInt(currentFilePart)));
