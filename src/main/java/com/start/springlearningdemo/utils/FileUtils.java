...
public class FileUtils {
   ...
   * @param length The number of bytes to write from the beginning of the buffer. This value should
   *     not exceed the buffer's length.
   * @param tempDirectoryPath The path to the directory where the file chunk will be created. The
   *     directory must exist.
   * @param splitPart The part number of the chunk. This is used in naming the file and helps in
   *     identifying the order of the chunks.
   * @return Path to the created file chunk.
   * @throws IOException IOException If an I/O error occurs writing to the file system
   */
  public static Path createFileChunk(
      final byte[] buffer, final int length, final Path tempDirectoryPath, final int splitPart)
      throws IOException {

    final Path tempFilePath =
        tempDirectoryPath.resolve(String.format("%s_%d_%s", SPLIT_PREFIX, splitPart, JSON_SUFFIX));
  ...
  public static List<Path> splitBySize(
      final InputStream largeFile, final int maxChunkSize, final Path tempDirectoryPath)
      throws IOException {
  ...
  public static File join(final List<File> list, final Path tempDirectoryPath) throws IOException {
    final Path outputFile =
        Files.createTempFile(tempDirectoryPath, FILE_UNSPLIT_DIRECTORY_PREFIX, JSON_SUFFIX);
